    ...
    (() => {
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                ...
                } else if (formRequiredItem.dataset.required === "linkedin") if (this.linkedinTest(formRequiredItem)) {
                    this.addError(formRequiredItem);
                    this.removeSuccess(formRequiredItem);
                    error++;
                } else {
            ...
            linkedinTest(input) {
                ...
                return !linkedinRegex.test(input.value.trim());
            }
        };
        function formSubmit() {
            ...
            async function formSubmitAction(form, e) {
                ...
                if (error === 0) if (form.hasAttribute("data-dev")) {
                    e.preventDefault();
                    formSent(form);
                } else {
                    e.preventDefault();
        ...
        da.init();
        if (isMobile.any()) {
            let cards = document.querySelectorAll(".item-approach");
            for (let card of cards) card.addEventListener("click", (e => {
                cards.forEach((c => c.classList.remove("active")));
                e.target.closest(".item-approach").classList.toggle("active");
            }));
        }
        window["FLS"] = false;
        menuInit();
